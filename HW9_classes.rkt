#|
CS 3520 Homework 9
Due: Friday, November 9th, 2018 11:59pm
Trenton Taylor
u0872466

Part 1 — Instantiating Object
In the starting code, {new Object} doen’t work, even though Object is supposed
to be a built-in class with no fields and no methods. Fix the implementation to
that {new Object} produces an instance of Object.
...
Add select:
  ...
  <Exp> = ...
        | {select <Exp> <Exp>}
The first expression in select should produce a number result, while the second
expression should produce an object. If the number is 0, select calls the zero
method of the object. If the number is not 0, select calls the nonzero method of
the object. In each case, select calls the method with the argument 0.
...
You will probably find it easiest to extend and test the Exp layer, then then
ExpI layer, and then the parse and interp-prog layer.
...
object that is an instance of the class named by <Sym> or any of its subclasses.
It should produce 1 f the result of <Exp> is any other object. Note that
{instanceof <Exp> Object} should produce 0 as long as the value of <Exp> is an
object.

Note that you will have to introduce a notion of superclasses into the Class
layer, even though method inheritance remains the job of the ClassI layer.
  ...
  (lambda (a classes this-val arg-val)
    (local [(define (recur expr)
      ...
      (type-case Exp a
        ...
        [(thisE) this-val]
        [(argE) arg-val]
        [(instanceofE obj-expr cn)
         (let ([robj (recur obj-expr)])
           (type-case Value robj
        ...
        [(selectE n obj)
         (let ([robj (recur obj)])
           (type-case Value robj
             [(objV class-name vals)
              (type-case Value (recur n)
                [(numV v)
                 (if (equal? v 0)
                     (call-method class-name 'zero classes robj (numV 0))
                     (call-method class-name 'nonzero classes robj (numV 0)))]
...
(module+ test
  (test/exn (interp (selectE (numE 7) (plusE (numE 7) (numE 7)))
                    empty
                    (objV 'A empty)
                    (numV 0))
            "not an object")
  (test/exn (interp (selectE (newE 'Object empty) (newE 'Posn
                                                        (list (numE 2)
                                                              (numE 7))))
                    (list posn-class)
                    (objV 'Object empty)
...
(define (exp-i->c [a : ExpI] [super-name : Symbol]) : Exp
  (local [(define (recur expr)
    ...
    (type-case ExpI a
      ...
      [(argI) (argE)]
      [(thisI) (thisE)]
      [(selectI n-expr obj-expr)
       (selectE (recur n-expr) (recur obj-expr))]
...
(module+ test
  (test (exp-i->c (selectI (numI 7)
                           (newI 'Object empty))
                  'Object)
        (selectE (numE 7)
                 (newE 'Object empty)))
  (test (exp-i->c (selectI (numI 7) (newI 'Yo empty))
                  'Object)
        (selectE (numE 7) (newE 'Yo empty)))
...
(module+ test
  (test (parse `{select 7 {new Yo 7 7}})
        (selectI (numI 7) (newI 'Yo (list (numI 7) (numI 7)))))
